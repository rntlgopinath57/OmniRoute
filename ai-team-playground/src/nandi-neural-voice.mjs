// NANDI_NEURAL_VOICE_V1
// NANDI_AUDIO_UNLOCK_V2 — persistent Web Audio graph; unlock once on a real gesture.
// Browser-local neural TTS. Kokoro is bundled at build time; model weights load
// from the Apache-2.0 ONNX model once and are cached by the browser.
import { KokoroTTS } from "kokoro-js";
import { env as transformersEnv } from "@huggingface/transformers";

const MODEL_ID = "onnx-community/Kokoro-82M-v1.0-ONNX";
const DEFAULT_VOICE = "bm_george";
const NANDI_HF_ORIGIN = "https://huggingface.co/";
const NANDI_HF_MODEL_PREFIX = "onnx-community/Kokoro-82M-v1.0-ONNX/resolve/";
const NANDI_HF_PROXY_PREFIX = "/api/hf/";

const nativeFetch = globalThis.fetch.bind(globalThis);
function proxiedFetch(input, init) {
  const raw = typeof input === "string" ? input : input?.url;
  if (typeof raw === "string" && raw.startsWith(NANDI_HF_ORIGIN + NANDI_HF_MODEL_PREFIX)) {
    const relative = raw.slice(NANDI_HF_ORIGIN.length);
    return nativeFetch(NANDI_HF_PROXY_PREFIX + relative, init);
  }
  return nativeFetch(input, init);
}
transformersEnv.fetch = proxiedFetch;
globalThis.fetch = proxiedFetch;

let model = null;
let loading = null;
let unavailable = false;
let lastError = "";
let generation = 0;
let progress = 0;
let device = "unknown";
let speakingText = "";
let audioContext = null;
let gainNode = null;
let currentSource = null;
let unlocked = false;

function emit(type, detail = {}) {
  window.dispatchEvent(new CustomEvent("nandi-voice", { detail: { type, ...detail } }));
}

function ensureAudioGraph() {
  if (audioContext && gainNode) return { context: audioContext, gain: gainNode };
  const AudioContextCtor = globalThis.AudioContext || globalThis.webkitAudioContext;
  if (!AudioContextCtor) return null;
  audioContext = new AudioContextCtor({ latencyHint: "interactive" });
  gainNode = audioContext.createGain();
  gainNode.gain.value = 0.96;
  gainNode.connect(audioContext.destination);
  return { context: audioContext, gain: gainNode };
}

async function unlock() {
  try {
    const graph = ensureAudioGraph();
    if (!graph) {
      lastError = "Web Audio API is unavailable in this browser";
      unlocked = false;
      emit("blocked", { error: lastError, engine: "kokoro-web-audio" });
      return false;
    }

    if (graph.context.state === "suspended") {
      await graph.context.resume();
    }

    if (graph.context.state !== "running") {
      lastError = "Audio context is not running";
      unlocked = false;
      emit("blocked", { error: lastError, state: graph.context.state, engine: "kokoro-web-audio" });
      return false;
    }

    // Exercise the already-resumed graph during the user gesture. Future Kokoro
    // audio can then start after async model/inference work without autoplay blocks.
    const silent = graph.context.createBuffer(1, 1, graph.context.sampleRate);
    const source = graph.context.createBufferSource();
    source.buffer = silent;
    source.connect(graph.gain);
    source.start(0);

    unlocked = true;
    lastError = "";
    emit("unlocked", { state: graph.context.state, engine: "kokoro-web-audio" });
    return true;
  } catch (err) {
    lastError = String(err?.message || err);
    unlocked = false;
    emit("blocked", { error: lastError, engine: "kokoro-web-audio" });
    return false;
  }
}

async function load() {
  if (model) return model;
  if (unavailable) return null;
  if (loading) return loading;

  const token = ++generation;
  loading = (async () => {
    try {
      device = "wasm";
      emit("loading", { progress: 0, device });
      const tts = await KokoroTTS.from_pretrained(MODEL_ID, {
        dtype: "q8",
        device,
        progress_callback: (p) => {
          const pct = typeof p?.progress === "number" ? p.progress / 100 : progress;
          progress = Math.max(progress, Math.min(1, pct || 0));
          emit("loading", { progress, device });
        },
      });
      if (token !== generation) return null;
      model = tts;
      progress = 1;
      emit("ready", { device, voice: DEFAULT_VOICE });
      return model;
    } catch (err) {
      lastError = String(err?.message || err);
      unavailable = true;
      emit("error", { error: lastError, device });
      return null;
    } finally {
      loading = null;
    }
  })();
  return loading;
}

function stopAudio() {
  const source = currentSource;
  currentSource = null;
  if (source) {
    try { source.onended = null; } catch {}
    try { source.stop(0); } catch {}
    try { source.disconnect(); } catch {}
  }
  speakingText = "";
}

function cancel() {
  generation++;
  stopAudio();
  emit("cancelled");
}

async function speak(text, opts = {}) {
  text = String(text || "").trim();
  if (!text) return false;

  const tts = await load();
  if (!tts) return false;

  const graph = ensureAudioGraph();
  if (!graph || !unlocked || graph.context.state !== "running") {
    lastError = graph
      ? `Audio context is ${graph.context.state}; user gesture required`
      : "Web Audio API is unavailable in this browser";
    emit("blocked", {
      error: lastError,
      state: graph?.context?.state || "unavailable",
      engine: "kokoro-web-audio",
    });
    return false;
  }

  const token = generation;
  try {
    const generated = await tts.generate(text, {
      voice: opts.voice || DEFAULT_VOICE,
      speed: typeof opts.speed === "number" ? opts.speed : 0.96,
    });
    if (token !== generation) return false;

    const blob = generated.toBlob();
    const bytes = await blob.arrayBuffer();
    if (token !== generation) return false;

    const audioBuffer = await graph.context.decodeAudioData(bytes.slice(0));
    if (token !== generation) return false;

    stopAudio();
    const source = graph.context.createBufferSource();
    source.buffer = audioBuffer;
    gainNode.gain.value = typeof opts.volume === "number" ? opts.volume : 0.96;
    source.connect(gainNode);
    currentSource = source;
    speakingText = text;

    return await new Promise((resolve) => {
      let finished = false;
      const done = (ok) => {
        if (finished) return;
        finished = true;
        if (currentSource === source) currentSource = null;
        try { source.onended = null; } catch {}
        try { source.disconnect(); } catch {}
        speakingText = "";
        resolve(ok);
      };

      source.onended = () => done(true);
      try {
        source.start(0);
        lastError = "";
        emit("start", {
          text,
          engine: "kokoro-web-audio",
          voice: opts.voice || DEFAULT_VOICE,
          duration: audioBuffer.duration,
        });
      } catch (err) {
        lastError = String(err?.message || err);
        emit("error", { error: lastError, engine: "kokoro-web-audio" });
        done(false);
      }
    });
  } catch (err) {
    lastError = String(err?.message || err);
    emit("error", { error: lastError, engine: "kokoro-web-audio" });
    return false;
  }
}

async function probe(text = "Nandi is ready.") {
  const tts = await load();
  if (!tts) return { ok: false, bytes: 0, error: lastError };
  try {
    const audio = await tts.generate(String(text || "Nandi is ready."), {
      voice: DEFAULT_VOICE,
      speed: 0.96,
    });
    const blob = audio.toBlob();
    return {
      ok: blob.size > 1000,
      bytes: blob.size,
      type: blob.type || "audio/wav",
      device,
      voice: DEFAULT_VOICE,
    };
  } catch (err) {
    lastError = String(err?.message || err);
    return { ok: false, bytes: 0, error: lastError, device, voice: DEFAULT_VOICE };
  }
}

function status() {
  return {
    ready: Boolean(model),
    loading: Boolean(loading),
    unavailable,
    progress,
    device,
    voice: DEFAULT_VOICE,
    unlocked,
    audioState: audioContext?.state || "not-created",
    speaking: Boolean(currentSource),
    speakingText,
    lastError,
  };
}

window.NandiNeuralVoiceVersion = "NANDI_NEURAL_VOICE_V1";
window.NandiNeuralVoice = { load, unlock, speak, cancel, probe, status };
